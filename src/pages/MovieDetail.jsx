import axios from "axios";
import React, { useEffect, useState, useContext } from "react";
import { useParams, Link, useNavigate } from "react-router-dom";
import { AuthContext } from "../context/AuthContext";
import VideoSection from "../components/VideoSection";
import { toastErrorNotify } from "../helper/ToastNotify";

const MovieDetail = () => {
  const { id } = useParams(); // Movie ID from URL params
  const [movieDetails, setMovieDetails] = useState(null); // Initialize as null for safe destructuring
  const [videoId, setVideoId] = useState(null);
  const { currentUser } = useContext(AuthContext); // Current logged-in user
  const navigate = useNavigate(); // Used for navigation

  const API_KEY = import.meta.env.VITE_TMDB_KEY;
  const movieDetailBaseUrl = `https://api.themoviedb.org/3/movie/${id}?api_key=${API_KEY}`;
  const videoUrl = `https://api.themoviedb.org/3/movie/${id}/videos?api_key=${API_KEY}`;
  const baseImageUrl = "https://image.tmdb.org/t/p/w1280";
  

  // Fetch movie details and video
  useEffect(() => {
    // If the user is not logged in, redirect to login page
    if (!currentUser) {
      toastErrorNotify("You must be logged in to view movie details!");
      setTimeout(() => navigate("/login"), 2000);
      return;
    }

    // Fetch movie details
    axios
      .get(movieDetailBaseUrl)
      .then((res) => setMovieDetails(res.data))
      .catch((err) => toastErrorNotify("Failed to fetch movie details"));

    // Fetch video ID
    axios
      .get(videoUrl)
      .then((res) => {
        if (res.data.results && res.data.results.length > 0) {
          setVideoId(res.data.results[0].key);
        }
      })
      .catch((err) => toastErrorNotify("Failed to fetch video details"));
  }, [id, currentUser, navigate]); // Dependencies include movie ID, user, and navigate

  // Loading or error state handling
  if (!movieDetails) {
    return <div className="text-center text-white py-10">Loading movie details...</div>;
  }

  const { title, poster_path, overview, vote_average, release_date, vote_count } = movieDetails;

  return (
    <div className="md:container px-10 mx-auto py-5">
      <h1 className="text-center text-white text-3xl">{title}</h1>
      {videoId && <VideoSection videoKey={videoId} />}
      <div className="md:container flex justify-center px-10">
        <div className="flex flex-col lg:flex-row max-w-6xl rounded-lg bg-gray-100 shadow-lg">
        {poster_path ? (
  <img
    className="lg:w-1/3 h-96 lg:h-[600px] object-cover rounded-t-lg md:rounded-none md:rounded-l-lg"
    src={baseImageUrl + poster_path}
    alt="poster"
    loading="lazy"
  />
) : (
  <div className="lg:w-1/3 h-96 lg:h-[600px] object-cover rounded-t-lg md:rounded-none md:rounded-l-lg">
    <span className="text-gray-500">No Poster Available</span>
  </div>
)}
          <div className="p-6 flex flex-col justify-between">
            <div>
              <h5 className="text-gray-900 text-xl font-medium mb-2">Overview</h5>
              <p className="text-gray-700 text-base mb-4">{overview}</p>
            </div>
            <ul className="bg-gray-100 rounded-lg border border-gray-400 text-gray-900">
              <li className="px-6 py-2 border-b border-gray-400 w-full rounded-t-lg">
                Release Date: {release_date}
              </li>
              <li className="px-6 py-2 border-b border-gray-400 w-full">Rate: {vote_average}</li>
              <li className="px-6 py-2 border-b border-gray-400 w-full">Total Vote: {vote_count}</li>
              <li className="px-6 py-2 border-gray-400 w-full rounded-t-lg">
                <Link
                  to="/"
                  className="text-blue-600 hover:text-blue-700 transition duration-300 ease-in-out mb-4"
                >
                  Go Back
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MovieDetail;
